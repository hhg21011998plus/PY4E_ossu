...
try:
    ...
    for line in fhand:
        ...
        if line.startswith("X-DSPAM-Confidence:"):
            ...
            floatInLine = line[colonPosition+1 : ]
            floatWeNeed = floatInLine.strip()
            totalFLoat += float(floatWeNeed)
            count += 1
    print(totalFLoat/count)
except:
    print("Not found this file:", inp)
...
try:
    if inp == "na na boo boo" :
        ...
        for line in fhand:
            count += 1
        print("There were %d subject lines in %s" % (count, inp))
except:
